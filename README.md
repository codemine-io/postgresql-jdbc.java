...
```java
// DEFAULT is already SERIALIZABLE with retries, so this works out of the box:
transferFunds.execute(jdbcConnection, TransactionSettings.DEFAULT);

// Optimize when you know weaker isolation is safe:
TransactionSettings optimized = TransactionSettings.DEFAULT
        .withIsolationLevel(IsolationLevel.READ_COMMITTED)
        .withMaxAttempts(1);

transferFunds.execute(jdbcConnection, optimized);
```
